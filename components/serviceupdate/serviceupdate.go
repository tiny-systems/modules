...
func (c *Component) handleRequest(ctx context.Context, handler module.Handler, req Request) module.Result {
	...
	if err := k8sClient.Get(ctx, client.ObjectKey{Namespace: req.Namespace, Name: req.Name}, svc); err != nil {
		return c.handleError(ctx, handler, req, fmt.Errorf("service not found: %w", k8s.ClassifyError(err)))
	}
	...
	if err := k8sClient.Update(ctx, svc); err != nil {
		return c.handleError(ctx, handler, req, fmt.Errorf("failed to update service: %w", k8s.ClassifyError(err)))
	}
